...
def csi_data_read_parse(port: str, csv_writer, log_file_fd):
    ...
    while True:
        ...
        strings = udp_server.recv_csi_raw_data
        if not strings:
            break
...
class Get_CSI_Data_SubThread (QThread):
    def __init__(self, serial_port, save_file_name, log_file_name):
        ...
        # 初始化udp_server
        try:
            self.udp_server = Udp_Server('ipv4', UDP_Server_IP, UDP_Server_Port)
            self.udp_server.socket_bind()
            self.udp_server.recv_data('raw_data')       
    ...
    def init_udp_server():
        try:
            self.dp_server = Udp_Server('ipv4', UDP_Server_IP, UDP_Server_Port)
            self.udp_server.socket_bind()
            self.udp_server.recv_data('raw_data')       
        except KeyboardInterrupt:
            print('Keyboard interrupt received. Closing socket and exiting.')
...
class Init_Graph_Window_SubThread(QThread):
    app = QApplication([])
...
if __name__ == '__main__':
    ...
    serial_port = 'COM10' #args.port
    file_name = 'csi_data.csv' #args.store_file
    log_file_name = 'csi_data.log' #args.log_file

    


    win_subthread = Init_Graph_Window_SubThread()
    win_subthread.start()

    csi_subthread = Get_CSI_Data_SubThread(serial_port, file_name, log_file_name)
    csi_subthread.start()
