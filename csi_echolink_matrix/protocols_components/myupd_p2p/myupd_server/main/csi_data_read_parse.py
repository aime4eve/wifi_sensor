...
def csi_data_read_parse(port: str, csv_writer, log_file_fd):
    ...
    while True:
        ...
        # 将字符串转换为 StringIO 对象，以便 csv.reader() 可以读取
        # 因为 csv.reader() 需要一个可迭代的对象作为参数，而字符串不是
        # StringIO 对象可以将字符串转换为一个可迭代的对象
        ...
        for i in range(csi_vaid_subcarrier_len):
            # 将 CSI 原始数据转换为复数，并存储到 csi_data_array 的最后一行
            # 原始数据中，实部和虚部是分开存储的，需要将它们组合成一个复数
            # csi_vaid_subcarrier_index[i] * 2 + 1 是实部的索引，csi_vaid_subcarrier_index[i] * 2 是虚部的索引
...
if __name__ == '__main__':
    ...
    log_file_name = "recv_csi_data.log"
